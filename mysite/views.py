    ...
    if check_token(request):
        ...
        if request.method=="POST":
            ...
            if user.pronum==0:                # 未加入项目且未创建项目
                ...
                user=models.Employee.objects.get(pronum=pro1.pronum)
    ...
    if check_token(request):
        ...
        if request.method=="POST":
            ...
            if flag:
                ...
                if fun.time_dif(pro[0].timeon,str(dayset.timeon))<0:
                    dayset.late=0
                dayset.save()
                return render(request, "mysite/index.html", {'message':'验证成功','user_url':user_url,'user_admin':user_admin})
            else:
                return  render(request, "mysite/check_in.html", {'user_url':user_url,'user_admin':user_admin,'message':'验证失败'})
        else:
...
def go_off(request):
    if check_token(request):
        if request.method=="GET":
            return render(request,'mysite/go_off.html')
        username = request.COOKIES.get('id')
        user = models.Employee.objects.get(username=username)
        pro=models.project.objects.get(pronum=user.pronum)
        now=datetime.now()
        dayset=models.dailycheck.objects.get(usernum=username,pronum=user.pronum)
        flag=1
